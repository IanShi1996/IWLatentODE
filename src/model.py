...
def rms_norm(tensor):
    ...
    Used as part of training speed-up from: https://arxiv.org/abs/2009.09457.
    Args:
        tensor (torch.Tensor): Input tensor.
    Returns:
        torch.Tensor: Root mean square of tensor.
    """
    return tensor.pow(2).mean().sqrt()


def make_norm(state):
    """Remove adjoint parameters from odeint_adjoint computation.
    Used as part of training speed-up from: https://arxiv.org/abs/2009.09457.
    """
...
class NeuralODE(nn.Module):
    ...
    def forward(self, z0, ts, rtol=1e-3, atol=1e-4):
        ...
        """
        z = odeint(self.nodef, z0, ts, rtol, atol, method='dopri5',
                   adjoint_options=dict(norm=make_norm(z0)))
...
class LatentNeuralODE(nn.Module):
    ...
    def reparameterize(self, qz0_mean, qz0_logvar):
        ...
        Args:
            q_mean (torch.Tensor): Mean of latent distribution.
            q_logvar (torch.Tensor): Log variance of latent distribution
        Returns:
    ...
    def get_elbo(self, x, pred_x, q_mean, q_var, noise_std=0.5, kl_weight=1.):
        ...
        pz0_mean = pz0_logvar = torch.zeros(q_mean.size(), device=x.device) + 1
...
class LatentNeuralODEBuilder():
    ...
    def build_latent_node(self):
        """Construct LatentNeuralODE with provided components.
        Returns:
            LatentNeuralODE: Constructed latent node.
        """
        return LatentNeuralODE(self.enc, self.latent_node, self.dec)
